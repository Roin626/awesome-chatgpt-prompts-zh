ChatGPT调教指令（prompt engineering），该方法通过为大语言模型提供上下文设定，明确模型的功能任务，为模型提供行为约束，从而有针对性的激活特定认为相关的知识节点，提高回答的相关性和准确性。通过指令,可以引导模型倾向于某些特定的回答方式或风格。指令中可以包含一些常见错误的警告,帮助模型避免这些陷阱。通过这些方式,调教指令能够有效地引导大语言模型,使其更好地专注于特定功能,提高回答的质量和相关性。
...
请作为一个富有经验的自媒体创作者，帮我写一份适用于ppt的文案，主题是XXX，内容要包括以下几点：
...
请按照以下规则给我发送图片： 1.使用markdown格式； 2.使用unsplash API； 3.使用" ![imgae]https://source.unsplash.com/featured/?<已翻译的英文内容> "格式回复； 4.不要使用代码块，不要描述其他内容，不要解释； 5.根据我输入的内容生成对应格式； 第一个输入内容：小狗在沙滩奔跑
